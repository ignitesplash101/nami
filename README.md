...
| **Honest uncertainty** | The headline P&L carries a **±1σ idiosyncratic dispersion band** (per-name residual vols from the beta regression, scaled to the median analog horizon — a floor, not a confidence interval) and a **severity ladder** — the exact worst / base / best engine P&L with every banded shock pushed to whichever analog-envelope edge is adverse (or favorable) for this book, making the envelope's severity cap visible instead of hidden inside one confident number. Results also disclose their evidence base: which shocks have **no enforced analog band**, when the scenario lands **outside its own analogs' replayed range**, and which names have **weakly determined betas** (dof-honest adjusted R² + effective-parameter counts). Backdated runs disclose that sample-book weights are **today's frozen snapshot** (point-in-time drift), with the snapshot date. |
...
| **Mobile-friendly** | Off-canvas rail drawer, responsive grids, 44px touch targets, vertical SSE stepper on phones. Playwright-swept at 320 / 390 / 768 / 844 / 1024 / 1280 / 1920 widths with horizontal-overflow, tap-target and text-zoom probes; pinch-zoom is never disabled and type scales with browser text-size settings (rem sizing). |

...
| **SSE progress streaming** | Real-time stepper through the 7-step pipeline (cache_check → market → analogs → envelope → narrative → betas → attribution) while a scenario runs. Cached runs are fast; external Yahoo/Gemini cold latency remains variable. |
...
| **Answer-first UX** | Every result leads with a plain-language **Impact summary** (portfolio P&L, top driver, active return, evidence count); quant terminology sits one tap away behind glossary-backed ⓘ explainers. The workbench is organized into **tabs** — *Scenario* / *Your book* (free, engine-only analytics) / *Library* (admin) — and the results drill layer splits into *Drivers / Positions / Story / Adjust (admin) / Advanced (admin)* under an always-visible answer band. Visitor mode is a clean **demo**: suggestion **chips** as the single sample-scenario selector, a compact first-run state, detailed metrics on drilldown. A ⌘K **command palette** accelerates every common action — an accelerator only, never the sole path to a control. |
...
| **Usage metrics (admin)** | An admin-only dashboard in the operations console: unique visitors, requests, runs by sample scenario and book, visitor-vs-admin split, cache-hit rate, errors and latency, plus the daily cost/quota trend. It **reads back the structured access log the app already writes** — no tracking cookie, no analytics vendor, no new stored identifier; hashed IPs are used only to count distinct visitors and never leave the server. |

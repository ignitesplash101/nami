...
| **Honest uncertainty** | The headline P&L carries a **±1σ idiosyncratic dispersion band** (per-name residual vols from the beta regression, scaled to the median analog horizon — a floor, not a confidence interval), and results disclose their evidence base: which shocks have **no enforced analog band**, when the scenario lands **outside its own analogs' replayed range**, and which names have **weakly determined betas** (dof-honest adjusted R² + effective-parameter counts). |

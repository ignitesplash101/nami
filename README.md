...
| **Analyst-grade tables** | Right-aligned **tabular numerals** across every table, sortable headers, per-table **CSV export** (semantic filenames, raw decimals, spreadsheet formula-injection guarded), an anchored scroll affordance on wide tables, and a wide-screen layout cap so 1920px monitors get readable line lengths instead of full-width sprawl. |

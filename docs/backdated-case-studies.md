...
| predicted top factor | VIX (+102% shock) |
| **predicted total P&L (naive)** | **−6.71%** |
| **predicted total P&L (Conditional Shapley explicit)** | **−6.91%** |
...
**Honest read**: nami captured the *direction* (−6.7% predicted vs −24.7% realized, both negative) and the *mechanism* (cyclical sectors lead, VIX surge, energy weakness). It badly *under-predicted magnitude* because the two selected analogs (China deval 2015, trade war 2018) were medium-stress events, not crash-grade. The Lehman GFC analog was eligible but not picked — the LLM's prompt-anchored "supply chains + recession scare" framing matched trade-war-shape events more than systemic-collapse events. Realized RTX (Raytheon) at −45.8% was driven by aerospace/defense exposure to travel collapse, which the abstracted prompt didn't surface.
...
| predicted top factor | VIX (+63% shock); XLK contrib −2.0% |
| **predicted total P&L (naive)** | **−7.96%** |
| **predicted total P&L (Conditional Shapley explicit)** | **−4.95%** |
...
**Honest read**: this case is the most "successful" of the three in qualitative terms. The selected analogs (Taper Tantrum 2013 + 2018 trade-war/risk-off) are the right reference set — both were rate-driven duration-derating events on growth equities, exactly the 2022 mechanism. The naive prediction (−8%) was 4× too small in magnitude, but the relative-ordering signal was correct: XLK shocked harder than SPY in the prediction, and NFLX/MSFT/NVDA/AMZN periphery shocks matched the realized loser hierarchy (NFLX was the worst, the model put it at −9.5% — directionally right, magnitude tiny). The 6-month realized window is also longer than the typical analog window (taper tantrum was ~3.5 months), so magnitude under-prediction is partly time-window mismatch.
...
| predicted top factor | VIX (+24% shock, deliberately downweighted by validator from 47% mean because `count < 3`) |
| **predicted total P&L (naive)** | **−1.75%** |
| **predicted total P&L (Conditional Shapley explicit)** | **−1.17%** |
...
**Honest read**: this case is **interesting precisely because nami got the sign wrong**. The model predicted −1.75% on the defensive_mix portfolio (small negative — appropriate magnitude for defensives during a banking shock). The realized return was *positive* +10.24%, driven by Eli Lilly's +38% standalone move on Mounjaro/GLP-1 momentum during this period, plus the broader "Fed will pivot dovish if banks break" trade that bid up rate-sensitive defensives. **nami's structural model correctly identified that defensives ride out banking stress better than financials**; it could not predict that an idiosyncratic LLY ramp would dominate the 2-month window. This is the right kind of "failure" — the engine's mechanism was sound, the realized result was driven by orthogonal news flow the engine had no mandate to model.

**Leakage notes**: most leaky of the three. The defensive_mix portfolio has minimal bank exposure, so this case is closer to "does the engine correctly identify that defensives ride out a banking stress" than to "does it predict the banking stress."
...
- **Periphery-ticker plausibility**: COVID picked XOM/GE/AAPL/JPM (energy, industrials, supply-chain, financials); 2022 picked the FAANG+ stack; SVB picked defensives. All thematically defensible.

## What they are NOT evidence of

- **Predictive power**. The magnitude error in Case 1 (~4× too small) and Case 2 (~4× too small) is substantial. nami's structural model treats factor shocks as drawn from the historical *envelope* of the selected analogs — if the selected analogs were a regime weaker than the realized regime, predictions will systematically under-magnify. This is by design; the alternative is unfounded magnitude extrapolation.
- **Calibration**. The Conditional Shapley explicit-only attribution sums to the factor-driven P&L (Case 1: −6.91% vs −6.71% naive; Case 2: −4.95% vs −7.96% naive — the divergence between naive and Shapley reflects correlation structure between the LLM-shocked factors, not a bug). For interpretability work the relative ordering of factor contributions matters more than the absolute sum.
